...
```
□ 信任边界已声明           →  如果没有，从第一层开始 (→ §5.1)
□ 7+ 类拒绝场景已定义       →  如果没有，补全拒绝模板 (→ §5.1)
□ 三层边界框架已实现        →  如果没有，这是最高优先级 (→ §6)
□ Kill Switch 已部署        →  如果没有，明天之前必须上线 (→ §7)
□ 工具调用日志已开启         →  如果没有，立即开启 (→ §8.1 监控层)
□ 审批门已就位              →  如果没有，先对破坏性操作加审批 (→ §8.1 架构层)
□ 会话限制已设置            →  如果没有，设置 60 次/30 分钟 (→ §8.1 架构层)
□ 红队测试定期进行           →  如果没有，安排本周第一轮 (→ §8.2)
```
