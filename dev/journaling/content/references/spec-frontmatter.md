...
> YAML frontmatter 的格式规范与字段说明。技能硬性要求：每个 journal 条目必须有 frontmatter 段（`---` 包裹的 YAML）——这是检索、组织和维护的基础。字段集由 journal 规则（规则文档的笔记元数据字段板块）定义；种子预设推荐方案（见 §2），可增删改——技能不强制任何具体字段。
>
> 使用场景：
> - 写新条目时参考字段格式
> - 考虑添加自定义字段时查阅指导原则
...
以下四个字段是种子预设的推荐方案——**不是技能硬性要求**（硬性仅"必须有 frontmatter 段"）。实际字段集由 journal 规则定义：规则文档若定义了笔记元数据字段板块，以规则为准；未定义时用本方案。`frontmatter` 脚本的 `check`/`replace` 默认按本字段集验证，可用 `--required-fields` 指定规则定义的字段集（见 `script-tools.md`）。
...
| 类型 | 数组[字符串] |
| 作用 | 检索入口 |
| 规范 | 遵循规则文档标签板块的规则；未定义标签规则时，可用种子预设标签集（见种子 RULES.md），或不用标签 |
...
| 字段 | 类型 | 场景 | 说明 |
|------|------|------|------|
| `status` | 字符串 | 进行中的任务条目 | 标记条目生命周期状态。推荐值：`active`（进行中）、`completed`（已完成）、`superseded`（已被替代）、`abandoned`（已放弃）。也可由各 journal 在规则文档中自行定义规则，或不使用此字段 |
...
- 例如一条进行中的任务条目如果暂时没有截止日期，可以写 `deadline:`（留空）而不是不写这个字段——这样保持结构一致性

### 4.5 按目录扩展字段

不同目录的内容性质不同，可以按目录引入不同的自定义字段。以下示例基于种子目录方案（`experience/`、`active_works/`、`knowledge/`——实际目录由 journal 规则定义）：
...
这不是硬性规定——根据实际需求灵活添加。如果你发现某个目录下的条目频繁使用同一组自定义字段，建议将该约定记录在规则文档中。

---

## 5. Field Ordering

种子预设字段按以下顺序排列，便于阅读和维护（字段集与顺序均可由 journal 规则调整）：
...
> 示例基于种子目录方案与种子字段集（实际目录与字段由 journal 规则定义）；示例中的自定义标签（如 `environment`）在定义标签规则的 journal 中需先按注册流程登记。
