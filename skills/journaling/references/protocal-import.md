...
1. **P1（准入判断）**——按照固定的顺序，依次做技术检查、直接通过、否决判断、读取判断。任何一个节点的 REJECT 信号即终止。
...
读取 `<journal-root>` 下的规则文档——必读，内容由 journal 自定（分类板块、标签板块、INDEX 结构板块、笔记元数据字段板块、写作需求板块等）。加载其中定义的规则并按之执行；为空或未定义相应板块时用种子方案（见 ../templates/seed/）。

规则文档不存在属异常（强制义务：必须存在）——此时从 ../templates/seed/ 复制规则文档种子到 `<journal-root>` 补齐（入口文件名由机制固定；只补缺失文件，不触发初始化协议的其他步骤，避免连带覆盖已有 INDEX）。

目录归属、标签、frontmatter 字段均遵循规则文档相应板块。
---
...
- 字段方案：按规则文档元数据板块执行；未定义时用种子推荐方案（见 ../templates/seed/ 规则文档种子的元数据字段板块）——不在本协议硬编码字段名。
- 格式验证：对照 `spec-frontmatter.md`（通用格式规范）。
- 已有有效 frontmatter → 验证格式，通过则不动；无效或无 → 按上述方案编写。`summary` 字段使用 P2-S2 编写的摘要。

完成后检查 frontmatter 格式是否正确。

### P3-S3: 建立发现链接

按 journal 规则确定发现入口位置，记录此文件的入口，确保它能在被需要时被发现。

### P3-S4: 检查加载链路

简单确认加载链路——下次 session 启动时能读到这个文件：

- 发现入口是否已按 journal 规则建立（指向此文件的链接或索引）
- 文件路径是否正确
