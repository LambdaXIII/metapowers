...
> 本文档是 journal 的 tag 受控词汇表。所有条目使用的标签必须来自此表。
> 这是一个成熟 tag 集示例（约 20 个 tag，按维度分组）。初始种子只有 7 个 tag，新 tag 随内容需求而增长。
> 在现行模型中，此内容位于规则文档（RULES.md）的标签板块——本示例展示该板块的成熟形态。

## Rules

- 每条条目至少选一个 tag
- project tag 可选，但如有明确项目归属则推荐添加
...
|-----|-------|
| `scriptum` | 项目 A 相关 |
| `metapowers` | Metapowers 项目 |
| `hermes-ops` | Hermes 运维相关 |
